...
AI 에이전트의 브라우저 작업 시 `agent-browser` CLI 사용.

```bash
# 사이트 열기 (--native: Rust 바이너리, --session-name: 세션 유지)
agent-browser --native --session-name PROJECT_NAME open https://example.com

# 인터랙티브 요소 스냅샷 (ref 번호 확인)
agent-browser --session-name PROJECT_NAME snapshot -i

# ref로 조작
agent-browser --session-name PROJECT_NAME click @e1
agent-browser --session-name PROJECT_NAME fill @e4 "value"

# 주석 스크린샷
agent-browser --session-name PROJECT_NAME screenshot --annotate ./check.png

# 종료
agent-browser --session-name PROJECT_NAME close
```

| 옵션 | 용도 |
|------|------|
| `--native` | Rust 바이너리 실행 (빠르고 메모리 적음) |
| `--session-name <name>` | 로그인/쿠키 상태 유지 |
| `snapshot -i` | 인터랙티브 요소만 표시 (AI가 ref로 조작) |
| `--annotate` | 스크린샷에 ref 번호 오버레이 |
